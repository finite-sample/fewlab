...
class BudgetConfig:
    ...
    n_items: int = 300
    p_features: int = 6
    budgets: List[int] = None  # Will test multiple budgets
    n_sims: int = 100
...
def generate_synthetic_data(
    cfg: BudgetConfig, rng: np.random.Generator
):
...
def compute_efficiency_metrics(results_df: pd.DataFrame) -> pd.DataFrame:
    ...
    summary = results_df.groupby(['K_budget', 'K_pct', 'method', 'coefficient']).agg({
        'error': 'var',
        'squared_error': 'mean',
    }).reset_index()
...
def plot_budget_sensitivity(summary: pd.DataFrame, cfg: BudgetConfig):
    ...
    avg_by_budget = summary.groupby(['K_budget', 'K_pct', 'method']).agg({
        'variance': 'mean',
        'rel_efficiency': 'mean',
    ...
    for method in methods + ['Random']:
        ...
        axes[0].plot(data['K_pct'], data['variance'], marker='o', label=method, linewidth=2, markersize=8)
    ...
    for method in methods:
        data = avg_by_budget[avg_by_budget['method'] == method]
        axes[1].plot(data['K_pct'], data['rel_efficiency'], marker='o', label=method, linewidth=2, markersize=8)
    axes[1].axhline(y=1.0, color='black', linestyle='--', label='Random baseline', linewidth=1.5)
    axes[1].set_xlabel('Budget (% of items)', fontsize=12)
    axes[1].set_ylabel('Relative Efficiency (vs. Random)', fontsize=12)
    axes[1].set_title('Relative Efficiency vs. Budget Size', fontsize=14, fontweight='bold')
    axes[1].legend()
    ...
    for method in methods:
        data = avg_by_budget[avg_by_budget['method'] == method]
        axes[2].plot(data['K_pct'], data['efficiency_gain_pct'], marker='o', label=method, linewidth=2, markersize=8)
...
def print_summary_table(summary: pd.DataFrame, cfg: BudgetConfig):
    ...
    for method in best_gain.index:
        ...
        print(f"  {method}:")
        print(f"    Best gain: {gain:+.2f}% at K={int(best_budget_row['K_budget'])} ({best_budget_row['K_pct']:.1f}% of items)")
